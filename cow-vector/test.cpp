...
TEST_CASE("Fast copying") {
    ...
    std::vector<COWVector<std::string>> vectors;
    for (auto i : std::views::iota(0, kSize)) {
        vectors.push_back(v);
...
TEST_CASE("Fast assignment") {
    ...
    std::vector<COWVector<std::string>> vectors(kSize);
    for (auto i : std::views::iota(0, kSize)) {
        vectors[i] = v;
...
TEST_CASE("Signature class") {
    STATIC_CHECK(std::is_copy_constructible_v<COWVector<std::string>>);
    STATIC_CHECK(std::is_copy_constructible_v<COWVector<std::string>>);
    STATIC_CHECK(std::is_assignable_v<COWVector<std::string>, const COWVector<std::string>&>);
    STATIC_CHECK(std::is_assignable_v<COWVector<std::string>, COWVector<std::string>&&>);
}